...
#include <string>
#include "X3DLib.h"
//void MultiTextureDesignPattern(int argc, char ** argv) {
X3D& X3D0 =  X3D();
...
Shape& Shape19 =  Shape();
Text& Text20 =  Text();
Text20.setString(new std::string[]{"Source shows design pattern for", "MultiTexture, MultiTextureCoordinate,", " and MultiTextureTransform nodes"}, 3);
CFontStyle& FontStyle21 =  CFontStyle();
FontStyle21.setContainerField("fontStyle");
FontStyle21.setJustify(new std::string[]{"MIDDLE", "MIDDLE"}, 2);
...
MultiTextureCoordinate26.setContainerField("texCoord");
...
MultiTexture32.setContainerField("texture");
MultiTexture32.setAlpha(0.8);
MultiTexture32.setColor(new float[]{0.9,1.0,0.2});
MultiTexture32.setFunction(new std::string[]{"COMPLEMENT", "ALPHAREPLICATE"}, 2);
MultiTexture32.setMode(new std::string[]{"MODULATE", "REPLACE", "BLENDDIFFUSEALPHA"}, 3);
MultiTexture32.setSource(new std::string[]{"DIFFUSE", "SPECULAR", "FACTOR"}, 3);
...
MultiTextureTransform36.setContainerField("textureTransform");
