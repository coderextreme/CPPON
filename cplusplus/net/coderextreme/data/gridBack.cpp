...
#include <string>
#include "X3DLib.h"
void gridBack(int argc, char ** argv) {
X3D& X3D0 =  X3D();
...
meta2.setContent(std::string("gridBack.x3d"));
head1.addMeta(&meta2);

meta& meta3 =  meta();
meta3.setName(std::string("description"));
meta3.setContent(std::string("Checkerboard grid background for X3D/VRML materials selection."));
head1.addMeta(&meta3);
...
meta6.setContent(std::string("8 April 2002"));
head1.addMeta(&meta6);

meta& meta7 =  meta();
meta7.setName(std::string("modified"));
meta7.setContent(std::string("12 January 2014"));
head1.addMeta(&meta7);
...
meta10.setContent(std::string("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials/gridBack.json"));
...
Material16.setDiffuseColor(new float[]{1.0,1.0,1.0});
