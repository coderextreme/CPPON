...
//#include "glut.h"
//#include "X3DLib.h"
int sliders(int argc, char ** argv) {
X3D& X3D0 =  X3D();
...
Layer17.setObjectType((std::string[]){"ALL"}, 1);
NavigationInfo& NavigationInfo18 =  NavigationInfo();
NavigationInfo18.setType((std::string[]){"EXAMINE"}, 1);
...
LayoutLayer72.setObjectType((std::string[]){"ALL"}, 1);
Layout& Layout73 =  Layout();
Layout73.setAlign((std::string[]){"LEFT", "BOTTOM"}, 2);
...
Text83.setString((std::string[]){"r = a + b * cos(c * theta + tdelta) * cos (d * phi + pdelta)"}, 1);
...
Text105.setString((std::string[]){"a="}, 1);
IS& IS106 =  IS();
...
Layer167.setObjectType((std::string[]){"ALL"}, 1);
